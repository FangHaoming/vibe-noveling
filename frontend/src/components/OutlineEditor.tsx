...
export default function OutlineEditor() {
  ...
  const loadFile = async () => {
    try {
      const data = await fileService.readFile('prompts/outline.md')
      setContent(data)
    } catch (error) {
  ...
  const handleSave = async () => {
    setSaving(true)
    try {
      await fileService.writeFile('prompts/outline.md', content)
      setSaved(true)
