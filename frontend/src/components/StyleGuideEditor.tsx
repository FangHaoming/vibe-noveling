...
export default function StyleGuideEditor() {
  ...
  const loadFile = async () => {
    try {
      const data = await fileService.readFile('prompts/style_guide.md')
      setContent(data)
    } catch (error) {
  ...
  const handleSave = async () => {
    setSaving(true)
    try {
      await fileService.writeFile('prompts/style_guide.md', content)
      setSaved(true)
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          编辑风格指南文件 (prompts/style_guide.md)
        </p>
        <button
          onClick={handleSave}
          disabled={saving}
